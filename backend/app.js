...
const { TimerEngine } = require('./lib/timer-engine');
const { QueueEngine } = require('./lib/queue-engine');
const { structuredError, createRequireAdmin, createLegacyRoute } = require('./lib/api-auth');
const { createHardware } = require('./lib/hardware');
const { createLogger } = require('./lib/logger');
const { registerTimerRoutes } = require('./lib/routes/timer');
const { registerRundownRoutes } = require('./lib/routes/rundown');
const { registerDisplayRoutes } = require('./lib/routes/display');
const { registerSystemRoutes } = require('./lib/routes/system');
const { createRundownCsvParser } = require('./lib/rundown-csv');
const { badRequest, enumField, numberField, requiredField, stringField, isAcceptedColorFormat } = require('./lib/validators');
const { detectHardware } = require('./runtime/detect-hardware');
...
let bindHost = process.env.BIND_HOST || '0.0.0.0';
...
const runtimeContext = () => ({
  ...
  runtime: detectedHardware.isElectron ? 'electron' : 'node',
  shell: runtimeShell,
  role: runtimeRole,
  hardwareProfile: detectHardwareProfile(detectedHardware, runtimeShell),
  detectedHardware,
...
function requireCapability(capability) {
  return (req, res, next) => {
    if (!runtimeCapabilities[capability]) {
      return res.status(404).json({ ok: false, code: 'CAPABILITY_NOT_AVAILABLE', capability, message: `${capability} is not available on this device` });
    }
    return next();
  };
}
...
function sanitizeDskBranding(branding) {
  ...
  const safe = {
    ...
    progressEnabled: input.progressEnabled === undefined ? defaults.progressEnabled : Boolean(input.progressEnabled),
...
app.post('/api/system/ap', requireAdmin, requireCapability('fallbackAccessPoint'), (req, res) => {
...
app.get('/api/system/ap/status', requireAdmin, requireCapability('fallbackAccessPoint'), (req, res) => {
  hardware.getApStatus((_, active) => res.json({ active }));
});

app.post('/api/system/wifi/scan', requireAdmin, requireCapability('wifiManagement'), (req, res) => {
...
app.post('/api/system/wifi/connect', requireAdmin, requireCapability('wifiManagement'), (req, res) => {
...
app.post('/api/system/wifi/static', requireAdmin, requireCapability('staticIpManagement'), (req, res) => {
...
registerRuntimeRoutes(app, { runtimeContext });
  ...
  return {
    ...
    start: () => new Promise((resolve) => {
      server.listen(listenPort, bindHost, () => {
        ...
        resolve({ url: `http://${bindHost}:${addr.port}`, port: addr.port, host: bindHost });
      });
    }),
